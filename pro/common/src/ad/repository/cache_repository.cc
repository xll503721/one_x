...
std::shared_ptr<AdSourceModel> CacheRepository::GetHighestPrice(const std::string& placement_id) {
    ...
    if (ad_source_model) {
        ad_source_service_->Show(ad_source_model, [=](std::shared_ptr<AdSourceModel> ad_source_model, std::shared_ptr<ONETEN::Error> error) {
            if (delegate_) {
        ...
        ad_source_service_->Click(ad_source_model, [=](std::shared_ptr<AdSourceModel> ad_source_model, std::shared_ptr<ONETEN::Error> error) {
            if (delegate_) {
                AdSDKDelegate::ActionType action = error ? AdSDKDelegate::ActionType::kClickFail : AdSDKDelegate::ActionType::kClickSuccess;
                delegate_->ActionCompletion(action, placement_id);
            }
        });
        ad_source_service_->Close(ad_source_model, [=](std::shared_ptr<AdSourceModel> ad_source_model, std::shared_ptr<ONETEN::Error> error) {
            if (delegate_) {
...
std::shared_ptr<AdSourceModel> CacheRepository::GetAnyOne(const std::string& placement_id) {
    ...
    return *cache_service_->GetAnyOne(placement_id)->GetSortAdSourceModelCache().begin();
}
