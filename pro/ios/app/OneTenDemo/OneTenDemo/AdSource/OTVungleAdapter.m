...
#import "OTVungleAdapter.h"

@interface OTVungleAdapter ()<VungleRewardedDelegate>

@end

@implementation OTVungleAdapter
    ...
    switch (styleType) {
        ...
        case OTAdSourceStyleTypeRewardedVideo: {
            if ([self.delegate.adSourceObject isKindOfClass:[VungleRewarded class]]) {
                return [(VungleRewarded *)self.delegate.adSourceObject canPlayAd];
            }
        }
            break;
...
- (void)registerWithUserInfo:(NSDictionary<id,id> *)userInfo { 
    [VungleAds initWithAppId:@"5e13cc9d61880b27a65bf735" completion:^(NSError * _Nullable error) {
...
- (void)loadRewardedVideoWithType:(OTAdSourceType)type userInfo:(NSDictionary<id, id> *)userInfo {
    VungleRewarded *rewardedAd = [[VungleRewarded alloc] initWithPlacementId:@"REWARDED03-2663131"];
